...
### A primeira vez que a regra nova valeu — e ela pegou

**09-09, à noite, no E42.** A regra que nasceu do erro da iteração 2 — *descrição
do Builder é alegação* — foi exercitada, e o Verifier rebaixou um PASS de gate
para `FAIL` **sem nenhum teste vermelho**: build verde, 684 unitários, 274
gametests, e os três casos novos sabendo falhar sob mutação própria dele.

O que ele reprovou foi **o que eu aleguei que eles fechavam**. O incidente do
fabricante que eu citei já tinha duas causas documentadas e corrigidas, nenhuma
no `WorkAssignment`; e eu tinha empurrado a metade da roça para um teste que não
a cobre. Ele foi ler o `REGRESSION-HISTORY.md`, o `ManufacturerWork` e o
`FarmPostponementTest` para descobrir isso — nenhuma das três coisas estava no
prompt que eu lhe dei.

**É o achado mais valioso que o mecanismo produziu até agora**, porque é a classe
de defeito que nenhuma bateria pega: teste que passa, documentação que promete o
que ele não protege, e a lacuna real seguindo aberta com aparência de fechada.
...
| ✅ | ~~**A reserva de tora não distingue espécie**~~ — **linha vencida, medida em 2026-09-11** | conta `WOOD` e `PLANKS` como grupos, e **está certo**: os dois estão em `INTERCHANGEABLE_IN_THE_WALL`, então a cerejeira substitui o carvalho na parede e o `MaterialChoice` a descasca para planta de carvalho desde 09-05. A conta e o construtor concordam, que é o que a Regra 27 pede. Separar por espécie recriaria a discordância de 09-10 — ver `ResourceSubstitution` |
