...
| S5 | **Arquivos acima de 500 linhas** | 🔨 **Medido de novo em 2026-08-26**, e os números de 08-15 estavam todos vencidos. Hoje são nove em `src/main`: `VillageDetectionHandler` 983, `BuildSiteScanner` 741, `TreeHarvester` 724, `BuilderWork` 670, `ConstructionPlanner` 654, `ManufacturerWork` 639, `MineDigging` 607, `RoadExtension` 563, `ColonySavedData` 502. **`LumberjackWork` saiu da lista** — foi de 1149 a 455 no corte do E19. Quatro entraram desde 08-15, e são os arquivos que a mina e a estrada trouxeram. A ADR-009 §6 continua valendo: **500 é indicador, e o corte é por responsabilidade** — nenhum destes é violação por si só |
...
```text
470 testes unitários     lógica pura do Core e serialização NBT
158 testes de jogo       a fronteira, num servidor sem cliente
```
...
| ~~D10~~ | ~~**E19**~~ ✅ **fechado em 2026-08-22** — corte por responsabilidade: `BuilderApproach`, `BuilderReport`, `MinerHaul`. ~~**E19** — `MinerWork` cruzou o teto de 500 linhas (465 → 511), e `BuilderWork` foi de 729 a 838 | Regressão de 2026-08-22. A ADR-009 §6 diz que 500 é indicador e o corte é por responsabilidade | §17 |
...
| ~~D9~~ | ~~**E18** — ninguém funde pedra~~ | ✅ **fechado em 2026-08-22** pelo sistema genérico que a ADR-009 pediu: `Production` declarada no recurso, e o fundidor perguntando ao livro de receitas | §17 |
