...
- [ ] 🟠 **Intermitencia de `SurfaceGatheringGameTest`:** reproduzida em 21-09 e de novo em 22-09. Na primeira de tres rodadas o fazendeiro criado fora da arena nao apareceu no `ServerWorld`; nas duas seguintes, o fundidor nao removeu a areia externa. Em 22-09, tres rodadas seguidas deram: nenhuma falha, depois **as duas juntas** (`farmerGathersDirt` e `smelterGathersGrass`), depois nenhuma outra vez — sem alteracao de codigo entre elas. Cairem juntas aponta raiz unica na fixture, e nao dois defeitos: os dois cenarios dependem de `FarthestVillageSector.farthestLoadedSector`, que le quais chunks estao carregados — estado global compartilhado com o resto da bateria. Investigar por ai antes de alterar coleta ou timeout.
